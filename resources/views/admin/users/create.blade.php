<x-admin-layout>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden max-w-2xl">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
            ...
            <p class="text-xs text-gray-500 mt-1">Gunakan form ini untuk membuat akun login baru. Untuk menambah data Guru/Staf secara administratif, silakan ke menu Data GTK.</p>
        </div>
        ...
        <div class="p-6">
            <form action="{{ route('admin.users.store') }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="email" name="email" id="email" class="w-full rounded-md border-gray-300 shadow-sm focus:border-school focus:ring focus:ring-school focus:ring-opacity-50" value="{{ old('email') }}" required>
                    @error('email') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                
                <div class="mb-6">
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-1">Password</label>
                    <input type="password" name="password" id="password" class="w-full rounded-md border-gray-300 shadow-sm focus:border-school focus:ring focus:ring-school focus:ring-opacity-50" required minlength="8">
