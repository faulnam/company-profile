<x-admin-layout>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden max-w-4xl">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
            <h2 class="text-lg font-bold text-gray-800">Form Edit Halaman Statis</h2>
        </div>
        ...
        <div class="p-6">
            <form action="{{ route('admin.pages.update', $page->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="title" id="title" class="w-full rounded-md border-gray-300 shadow-sm focus:border-school focus:ring focus:ring-school focus:ring-opacity-50" value="{{ old('title', $page->title) }}" required>
                    <p class="text-xs text-gray-500 mt-1">Slug/URL akan digenerate otomatis berdasarkan judul.</p>
                    @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Gambar Header Saat Ini</label>
                    @if($page->image)
                        <img src="{{ $page->image }}" class="w-64 h-32 object-cover rounded border border-gray-200 mb-3">
                    @else
                        <div class="w-64 h-32 bg-gray-100 rounded border border-gray-200 mb-3 flex items-center justify-center text-gray-400">No Image</div>
                    @endif
                ...
                <div class="mb-6">
                    ...
                    <textarea name="content" id="content" rows="10" class="w-full rounded-md border-gray-300 shadow-sm focus:border-school focus:ring focus:ring-school focus:ring-opacity-50" required>{{ old('content', $page->content) }}</textarea>
