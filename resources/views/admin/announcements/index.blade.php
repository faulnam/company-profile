<x-admin-layout>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            ...
            <a href="{{ route('admin.announcements.create') }}" class="bg-school hover:bg-school-dark text-white px-4 py-2 rounded text-sm font-medium transition flex items-center">
        ...
        <div class="px-6 py-4 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <form action="{{ route('admin.announcements.index') }}" method="GET" class="flex items-center w-full md:w-auto">
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    ...
                    <tr class="hover:bg-gray-50/50 transition">
                        <td class="py-3 px-4 font-bold text-gray-800">
                            {{ $announcement->title }}
                            <div class="text-xs text-gray-500 font-normal mt-1">{{ Str::limit(strip_tags($announcement->content), 50) }}</div>
                        </td>
                        <td class="py-3 px-4 text-gray-600">{{ $announcement->created_at->format('d M Y') }}</td>
                        <td class="py-3 px-4">
                        ...
                        <td class="py-3 px-4 text-center">
                            <div class="flex items-center justify-center space-x-2">
                                <a href="{{ route('admin.announcements.edit', $announcement->id) }}" class="text-blue-500 hover:text-blue-700 p-1">
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="py-6 text-center text-gray-500">Belum ada data pengumuman.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
