<x-admin-layout>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden max-w-4xl">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
            <h2 class="text-lg font-bold text-gray-800">Form Edit Kelas</h2>
        </div>
            ...
            <form action="{{ route('tu.classrooms.update', $classroom->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $classroom->name) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-school focus:ring-1 focus:ring-school" required>
                    ...
                    <div>
                        ...
                        <select name="major_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-school focus:ring-1 focus:ring-school" required>
                            ...
                            @foreach($majors as $major)
                                <option value="{{ $major->id }}" {{ old('major_id', $classroom->major_id) == $major->id ? 'selected' : '' }}>{{ $major->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="mb-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        @php
                            $selectedSubjects = $classroom->subjects->pluck('id')->toArray();
                        @endphp
                        ...
                        <label class="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition">
                            <input type="checkbox" name="subjects[]" value="{{ $subject->id }}" 
                                ...
                                class="w-4 h-4 text-school bg-gray-100 border-gray-300 rounded focus:ring-school">
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                    ...
                    <button type="submit" class="bg-school hover:bg-school-dark text-white px-5 py-2 rounded-lg text-sm font-medium transition">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
